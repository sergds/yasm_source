...
void CHudSuitPower::Paint()
{
	...
	surface()->DrawPrintText(L"DURACELL TURBOMAX", wcslen(L"DURACELL TURBOMAX"));
	...
	if ( m_iActiveSuitDevices )
	{
		...
		if (pPlayer->IsBreatherActive())
		{
			...
			surface()->DrawPrintText(L"CO2", wcslen(L"CO2"));
			ypos += text2_gap;
		}
		...
		{
			surface()->DrawSetTextPos(text2_xpos, ypos);
			surface()->DrawPrintText(L"SUPER COOL LED LIGHT", wcslen(L"SUPER COOL LED LIGHT"));
			ypos += text2_gap;
		}
		...
		{
			surface()->DrawSetTextPos(text2_xpos, ypos);
			surface()->DrawPrintText(L"RUN FOREST, RUN", wcslen(L"RUN FOREST, RUN"));
			ypos += text2_gap;
		}
	}
}
