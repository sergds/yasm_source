...
void CGameMovement::WaterMove( void )
{
	...
	{
		if(iJumps > 0){
			addspeed = (wishspeed * iJumps) + iJumps;
		}else{
			addspeed = wishspeed;
		}
		if (addspeed > 0)
		{
			...
			for (i = 0; i < 3; i++)
			{
				float deltaSpeed = accelspeed * wishvel[i] * (iJumps + 1);
...
void CGameMovement::FullWalkMove( )
{
	...
	// Not fully underwater
	{
		...
		if(!bJumping && iJumps > 0){
		Msg("%i Jumps Without Dying.Resseting\n", iJumps);
		iJumps = 0;
		}
...
bool CGameMovement::CheckJumpButton( void )
{
	...
	if ( gpGlobals->maxClients == 1 )
	{
		...
		// Add Speedo
		if(iJumps >= 1){
			flSpeedAddition += flNewSpeed * iJumps; }
